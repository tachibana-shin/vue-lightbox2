<template>
  <div class="lightbox" v-if="items.length > 0">
    <div class="lightbox--grid" :class="[
      ...
      <a class="lightbox--grid__item" v-for="(item, index) in items" :href="item" @click.prevent="showImage(index)" v-if="index < cells" :style="getBackgroundStyle(item)" @click="$emit('click:item', item, index)">
    ...
    <transition enter-active-class="lightbox--animate lightbox--animate__fadeIn" leave-active-class="lightbox--animate lightbox--animate__fadeOut" v-if="!hidePopup">
      <div class="lightbox--modal" v-if="openModal">
        ...
        <button class="lightbox--modal__btn lightbox--modal__btn-outline-primary lightbox--modal__btn-prev" @click="prev; $emit('click:prev')">
        ...
        <button class="lightbox--modal__btn lightbox--modal__btn-outline-primary lightbox--modal__btn-next" @click="next; $emit('click:next')">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 192 512" width="2em" height="2em" fill="currentColor">
